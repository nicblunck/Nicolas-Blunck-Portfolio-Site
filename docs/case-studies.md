...
- `competencies` keys must match an entry in
  ...
  silently dropped.
- Only `featured: true` cases appear in the homepage `WorkSection` and as
  "related cases" on other case pages — every case still gets its own
  `/work/<slug>` page either way.
...
2. Write `case.md` using the format above.
